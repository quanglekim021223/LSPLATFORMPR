...
async def ingest_skill_inventory(
    ...
    *,
    ...
    managed_incremental = skill_profile_modified_since is None and search_text is None
    ...
    while True:
        ...
        has_next_page = await _ingest_inventory_page(
            settings,
            client,
            checkpoints,
            writer,
...
async def _ingest_inventory_page(
    settings: Settings,
    client: SkillUpClient,
    checkpoints: CheckpointStore,
    writer: BronzeWriter,
    ...
    try:
        ...
        await _record_inventory_failure(checkpoints, client, run_id, page_number, exc)
        raise


async def _record_inventory_failure(
    checkpoints: CheckpointStore,
    client: SkillUpClient,
    run_id: str,
    page_number: int,
    ...
    await checkpoints.record_failed_page(run_id, DOMAIN, page_number, message, retryable=retryable)
